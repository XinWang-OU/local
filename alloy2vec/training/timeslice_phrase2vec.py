...
from wx_process_parallel import get_sentences, save_sentences
import time
import pandas as pd
...
def load_and_filter_corpus(corpus_path, year_range):
    # load the corpus and first line is header
    corpus = pd.read_csv(corpus_path, sep='\t', header=0)
    print(f"Original number of lines : {len(corpus)}")
    
    # transform the year column to numeric
    corpus['PY'] = pd.to_numeric(corpus['PY'], errors='coerce')
    # delete the rows with missing year
    corpus.dropna(subset=['PY'], inplace=True)
    corpus['PY'] = corpus['PY'].astype(int)
    
    print(f"number of lines after transfer: {len(corpus)}")
    
    filtered_texts = filter_corpus_by_year(corpus, year_range)
    print(f"number of lines after filter: {len(filtered_texts)}")
    return filtered_texts

def filter_corpus_by_year(corpus, year_range):
    if corpus.empty:
        return []
    # filter the corpus by year
    filtered_corpus = corpus[(corpus['PY'] >= min(year_range)) & (corpus['PY'] <= max(year_range))]
    if filtered_corpus.empty:
        return []
    # return texts
    return filtered_corpus['TI AB'].tolist()

def parse_year_ranges(year_ranges_str):
    """Parse year ranges string into a list of (start_year, end_year) tuples."""
    year_ranges = [tuple(map(int, range_.split('-')[::-1])) for range_ in year_ranges_str.split(',')]
    return year_ranges
...
if __name__ == "__main__":
    ...
    parser.add_argument("--year_ranges", default="", help="Year ranges for corpus division, formatted as YYYY-YYYY, separated by commas.")
    parser.add_argument("--model_name", required=True, help="Name for saving the model (in the timeslice_models folder).")
    ...
    parser.add_argument("--workers", default=32, help="Number of workers.")
    ...
    if args.year_ranges:
        year_ranges = parse_year_ranges(args.year_ranges)
        print("year_ranges: ",year_ranges)
    else:
        corpus = pd.read_csv(args.corpus, header=None, names=['year', 'text'])
        year_ranges = [(corpus['year'].min(), corpus['year'].max())]

    year_ranges = parse_year_ranges(args.year_ranges)
    print("year_ranges1: ",year_ranges)
    for year_range in year_ranges:
        sentences_timeslice = load_and_filter_corpus(args.corpus, year_range=year_range)
 ##############################################################################
          # For test
        if year_range == (None, None):
            file_name = f"{args.model_name}_all_years.txt"
        else:
            file_name = f"{args.model_name}_{year_range[0]}-{year_range[1]}.txt"
        save_path = os.path.join("timeslice_models", file_name)
        with open(save_path, 'w', encoding='utf-8') as f:
            for sentence in sentences_timeslice:
                f.write(sentence + '\n') 
        print(f"Saved sentences for {year_range} to {save_path}")
 ##############################################################################
    
        sentences = get_sentences(save_path)
        output_path = os.path.join("timeslice_models", f"{args.model_name}_{year_range[0]}_{year_range[1]}_processed1.txt")
        ...
        path = os.path.join("timeslice_models", f"{args.model_name}_{year_range[0]}_{year_range[1]}_processed2.txt")
        ...
        sentences, phraser = wordgrams(processed_sentences,
                              ...
                              ip=INCLUDE_PHRASES)
        phraser.save(os.path.join("timeslice_models", args.model_name + "_phraser.pkl"))
        ...
        my_model = Word2Vec(
            ...
            callbacks=callbacks)
        my_model_name = f"{args.model_name}_{year_range[0]}_{year_range[1]}"
        my_model.save(os.path.join("timeslice_models", my_model_name))
        ...
        print(f"Model for years {year_range[0]}-{year_range[1]} saved as {my_model_name}")
