...
for model, time in zip(models, time_periods):
    ...
    if not existing_keywords:
        similarity_over_time[time] = [('N/A', 0)]
        continue

    try:
        # 使用存在于模型中的关键词进行查询，直接保存查询结果
        top_context_words = model.wv.most_similar(positive=existing_keywords, topn=5000)
        similarity_over_time[time] = top_context_words
    except KeyError:
        similarity_over_time[time] = [('N/A', 0)]

key_word_str = "_and_".join(keywords)
output_file_path = os.path.join(output_dir, f"{key_word_str}_similarity_nofilter.txt")
...
with open(output_file_path, 'w', encoding='utf-8') as file:
    ...
    # 遍历每个时间段和对应的相似度列表
    for time_period, similarities in similarity_over_time.items():
